...
/* Слайдер первого экрана. Работы перелистываются по одной; автопрокрутка
   идёт, только пока экран виден и вкладка активна, и останавливается под
   курсором и на фокусе — иначе читатель возвращается к случайному кадру.
   При prefers-reduced-motion автосмены нет вовсе: только стрелки. */
(() => {
  ...
  const calm = window.matchMedia('(prefers-reduced-motion: reduce)');
  const pad = (n) => String(n).padStart(2, '0');
  let at = 0, timer = 0, visible = true, held = false;

  const paint = () => {
    slides.forEach((s, i) => s.toggleAttribute('data-on', i === at));
    if (count) count.textContent = pad(at + 1) + ' / ' + pad(slides.length);
  };
  const go = (step) => { at = (at + step + slides.length) % slides.length; paint(); };

  const stop = () => { if (timer) { window.clearInterval(timer); timer = 0; } };
  const start = () => {
    stop();
    if (calm.matches || !visible || held) return;
    timer = window.setInterval(() => go(1), 6000);
  };

  prev?.addEventListener('click', () => { go(-1); start(); });
  next?.addEventListener('click', () => { go(1); start(); });
  hero.addEventListener('pointerenter', () => { held = true; stop(); });
  hero.addEventListener('pointerleave', () => { held = false; start(); });
  hero.addEventListener('focusin', () => { held = true; stop(); });
  hero.addEventListener('focusout', () => { held = false; start(); });
