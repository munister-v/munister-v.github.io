...
`js/`: `app.js` (связка, меню, редактор полей) · `model.js` (Store, undo) · `autodef.js` (автовизначення по імені, транслітерація, логічна → фізична) · `diagram.js` (SVG) · `diagrams-ui.js` (вкладки діаграм) · `panel.js` · `sidebar.js` · `palette.js` (⌘K) · `ddl-parse.js` / `ddl-gen.js` (Oracle DDL) · `sqlite-gen.js` / `sandbox.js` (SQL-пісочниця: модель → SQLite DDL → sql.js) · `diff.js` (миграции) · `checks.js` · `docs.js` · `storage.js` / `workspace.js` (проекты, версии) · `templates.js` · `i18n.js` (uk/en)
