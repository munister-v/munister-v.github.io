...
import { t as tr, getLang } from './i18n.js?v=202609171543';
  ...
  <section id="${anchor(tb.name)}">
    ...
    <table>
      ...
      <tbody>${columns.map(({ n, c, keys, ref }) => `
        <tr><td class="n">${n}</td><td class="code${c.pk ? ' pk' : ''}">${esc(c.name)}</td><td class="code">${esc(c.type)}</td>
    ...
    ${uniques.length || indexes.length || outFks.length || inFks.length ? `<dl>
